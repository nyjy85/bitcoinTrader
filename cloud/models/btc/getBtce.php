<?php
	...
	function getCData(){
		...
		$price = dbMassData("SELECT * FROM (SELECT * FROM btcec WHERE price != 0 AND bid != 0 ORDER BY timestamp DESC LIMIT 240) t ORDER BY timestamp ASC"); //gets the last 240 rows then sorts them in asecending order
